...
- **H-scene images use `ex.moonchan.xyz` external links, NOT `/game-cgs/`**: the project's CG source is the ExHentai gallery (`ex.moonchan.xyz/s/<hash>/<id>?redirect_to=image`). External images work in DOM `<img>` without CORS. Never default to local `game-cgs` (that was the old AVD approach — user explicitly rejects it). When adding/migrating scenes, inline the ex URLs into the script data.
- **VN scene scripts / image maps must NOT go into the main bundle**: static `import` in `examples.ts` pulls all scene scripts + image URL maps into the 1.9MB main bundle (and triggered the 504). Use `React.lazy(() => import('./path/X'))` for every example component so each scene is a separate chunk loaded only on its `#hash`. Heavy chunks: `imageMap*`, `scripts-*`, `AvdController-*`.
- **VN framework is being reworked on branch `vn-rework` (script-driven, decoupled)**: the new `src/vn/` player is JSON-script driven — lines are `preload` (with `{key,url}` + `wait`), `say`, `bg`, `choice`, `jump`, `label`, `end`. Per-script loading with `wait` (load-then-continue) vs `wait:false` (continue while loading). Old `src/avd/*` is legacy; do not extend it.
