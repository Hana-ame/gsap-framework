...
- **硬编码 4 tiles（2×2）→ zoom 小时 viewport 覆盖不全**：原 `buildViewport` 永远创建 4 个 tile，每个 tile 覆盖 `worldW × worldH`。默认 cellSize=8 → worldW=1600。但 zoom 到 cellSize=4 时 worldW=800，viewportW 可能 1920。2×2 tiles = 1600 宽，viewport 1920 宽 → 右边 320px 空白。**第一轮修**（commit `9679756`）：dynamic tile grid — `tileCols = ceil(viewportW / worldW) + 1; tileRows = ceil(viewportH / worldH) + 1`。`updateTilePositions` 用 `tileCols` 做 wrap。**第二轮修**（commit `3093dcd`）：`setCellSize` / `setRows` / `setCols` 改了 worldW/worldH 后**没有 rebuild tile grid**——tileCols/tileRows 停留在旧值，zoom 缩小后 tile 数不够，宽屏（2560+）上右边缘仍然空白。**修法**：抽 `rebuildTileGrid(refs)`（销毁旧 tiles → 重算 tileCols/tileRows → 创建新 tiles → 定位 → 重画），在 `setCellSize` / `setRows` / `setCols` 和 `buildViewport` 里都调它。公式对但重建时机漏了——算一次但永不更新。 **踩了两次**：第一次改了公式但没改 setCellSize（右边还穿），第二次才补上 rebuild 调用。
...
- **SW cache 撞 stale（sim-v3 → sim-v4 → sim-v5 三次都踩）**：`networkFirst` for navigation + `cacheFirst` for assets + `skipWaiting` + `controllerchange` 强制刷新。**真实症状（sim-v5 这次）**：用户报「黑屏」，DevTools console 报 `Failed to load module script: Expected a JavaScript-or-Wasm module script but the server responded with a MIME type of "text/html"`。**根因**：旧 SW (sim-v4) 仍在用户浏览器运行；activate handler 只删 `!= CACHE` 的 cache，所以 `sim-v4` 和 `sim-v4-assets` 一直留着；旧 SW 仍 serve precached 旧 `/index.html`；旧 HTML 引用旧 bundle hash；新 deploy 已经把那个 hash 的 bundle 文件删了；浏览器 fetch 旧 hash → Cloudflare 走 SPA fallback 返回 `index.html` (Content-Type: `text/html`)；浏览器对 `<script type="module">` 强制 MIME 检查 → 拒绝解析 → JS 永远不执行 → root div 空 → 黑屏（连 `__paintError` 红面板都来不及 paint）。**修法**：bump `CACHE = 'sim-vN'`（commit `06fff7e` = sim-v5）。SW byte 变 → 浏览器装新 SW → activate handler 删 sim-v(N-1) + sim-v(N-1)-assets → 重新 precache 新 HTML → controllerchange → 自动 reload → 干净。**预防**：(1) **每次重大 deploy 顺手 bump SW 版本**，把「bump SW」放进 deploy checklist；(2) `index.html` 的 `__paintError` 是防「JS 跑后抛」，但「JS 根本不跑」要靠 SW 自检 / DevTools `Application → Service Workers` 确认版本。
