...
export class DomDisplayObject {
  ...
  get scale(): { x: number; y: number } {
    const self = this;
    return {
      get x() { return self._transform.scaleX; },
      set x(v: number) { self._transform.scaleX = v; self._updateTransform(); },
      get y() { return self._transform.scaleY; },
      set y(v: number) { self._transform.scaleY = v; self._updateTransform(); },
    };
  }
  set scale(v: { x: number; y: number }) {
    this._transform.scaleX = v.x;
    this._transform.scaleY = v.y;
