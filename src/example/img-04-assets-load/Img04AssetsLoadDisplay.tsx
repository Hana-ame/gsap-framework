...
export function Img04AssetsLoadDisplay() {
  useEffect(() => {
    const stop = startPixiApp((proxy: SubCanvasProxy) => {
      ...
      const label = new PIXI.Text({
        text: 'Step 04: Assets.load(url) — loading\u2026',
      ...
      PIXI.Assets.load(URL).then((tex) => {
        ...
        label.text = 'Step 04: Assets.load(url) — OK';
      }).catch((e) => {
        label.text = `Step 04: FAILED — ${e}`;
      });
    });
    return () => stop();
  }, []);
...
Img04AssetsLoadDisplay.head = {
  title: 'Img 04: Assets.load(url)',
  description: 'PIXI.Assets.load(url) — v8 标准加载方式',
};
