...
export function ComponentAvdT1Display() {
  ...
  useEffect(() => {
    ...
    destroyRef.current = startPixiApp((proxy: SubCanvasProxy) => {
      ...
      const allBgKeys = T1_LINES.filter(l => l.bgKey).map(l => l.bgKey!);
      preloadImages(allBgKeys).then(() => {
        return parseScript({ lines: T1_LINES, roster: {} }, makeResolver());
      }).then((parsed) => {
...
ComponentAvdT1Display.head = {
  title: 'T1 オナニー',
  description: '「イルと貧乳の国」オナニーシーン',
};
