import { useEffect, useRef } from 'react';
import { DomContainer, DomText } from '../../avd/dom/DomNode';
import { DomDialogueBox } from '../../avd/dom/DomDialogueBox';
import { DomTypingEngine } from '../../avd/dom/DomTypingEngine';

export function Step03DomTypingDisplay() {
  const rootRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const el = rootRef.current;
    if (!el) return;

    const root = new DomContainer();
    root.el.style.position = 'relative';
    root.el.style.width = '100%';
    root.el.style.height = '100%';
    el.appendChild(root.el);

    const db = new DomDialogueBox(root, {
      boxX: 40, boxY: 300, boxWidth: 800, boxHeight: 180,
      boxRadius: 10, boxPadding: 20,
      boxBg: 0x0a0a14, boxBgAlpha: 0.88,
      nameColor: 0x88aaff, nameSize: 15,
      fontFamily: 'serif', arrowColor: 0x88aaff,
    });

    db.setSpeaker('Narrator');

    const engine = new DomTypingEngine();
    const container = engine.start(
      '夜色笼罩着古老的小镇。你站在岔路口前。',
      40,
      { fontFamily: 'serif', fontSize: 18, fill: '#d8d8e8', wordWrapWidth: 760, lineHeight: 25 },
      760,
      25,
    );

    db.setTextContainer(container);

    let rafId: number;
    const tick = () => {
      engine.update(16);
      rafId = requestAnimationFrame(tick);
    };
    rafId = requestAnimationFrame(tick);

    return () => {
      cancelAnimationFrame(rafId);
      engine.destroy();
      db.destroy();
      root.destroy();
    };
  }, []);

  return (
    <div ref={rootRef} style={{ position: 'fixed', inset: 0, background: '#111122' }} />
  );
}

Step03DomTypingDisplay.head = {
  title: 'Step 03: DomTypingEngine',
  description: '对话框 + 打字机效果 — 逐字显示',
};
