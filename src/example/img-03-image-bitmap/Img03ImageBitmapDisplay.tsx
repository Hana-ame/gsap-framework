...
export function Img03ImageBitmapDisplay() {
  useEffect(() => {
    const stop = startPixiApp((proxy: SubCanvasProxy) => {
      ...
      const label = new PIXI.Text({
        text: 'Step 03: fetch → createImageBitmap — loading\u2026',
      ...
      (async () => {
        try {
          ...
          label.text = 'Step 03: fetch → createImageBitmap — OK';
        } catch (e) {
          label.text = `Step 03: FAILED — ${e}`;
        }
      })();
    });
    return () => stop();
...
Img03ImageBitmapDisplay.head = {
  title: 'Img 03: ImageBitmap',
  description: 'fetch → blob → createImageBitmap → Texture.from(bitmap)',
};
