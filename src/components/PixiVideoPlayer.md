...
17. **React Strict Mode 二次挂载触发 Chromium 媒体 Cache Lock 死锁**（critical，dev 模式重复进入视频不显示）：Strict Mode 的 `Mount → Unmount → Mount` 同步序列中，cleanup 调 `player.destroy()` → 内部 `htmlVideo.removeAttribute('src') + load()`（**以及** `videoTexture.destroy(true)` cascade 到 PIXI `VideoSource.destroy()` 内部的 `source.src = "" + source.load()`）→ 同步向浏览器网络栈发 Abort 信号截断第一道媒体流。几乎同时，Mount 2 发起对**同一 URL** 的新请求。Chromium 网络缓存因上次 Abort 产生竞态闭塞，第二道请求永远 Pending，`readyState=0`，`canplay` 一辈子不触发，`initVideoSource` 不跑，`videoTexture` 没建，画面死寂。**修法**：把整个媒体清理（texture destroy + DOM remove + src/load + blob revoke）defer 到 `setTimeout(() => {...}, 0)`，让 Mount 2 的新 `<video>` 先把请求入队，下个 macrotask 再 Abort 旧实例，浏览器可正常共享网络管道。**同步部分保留**：pause、removeEventListener、scene graph 摘除、closure ref 置空。
