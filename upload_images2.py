"""Batch upload H-scene images - v2 with correct HTML extraction + retry."""
import pickle, requests, time, re, json, sys
...
def main():
    ...
    name_to_idx = {name: idx for idx, name in enumerate(all_fnames)}

    load_cookies = lambda s: [s.cookies.set(c.name, c.value, domain=c.domain, path=c.path) for c in pickle.load(open(COOKIE_FILE, 'rb'))]
    ...
    for fname in TARGET_FILES:
        ...
        token = tokens[idx]
        page_num = idx + 1
        page_url = f'{BASE}/s/{token}/3631029-{page_num}'

        print(f"[{idx:3d}] {fname} (page {page_num})...", end=' ', flush=True)
        ...
        for attempt in range(3):
            try:
                ...
                s.headers.update({
                    ...
                    'Referer': f'{BASE}/g/3631029/8a2a08b248/',
                })

                r = s.get(page_url, timeout=30)
                if r.status_code != 200 or len(r.text) < 500:
                    print(f"page={r.status_code}", end=' ')
                    time.sleep(2)
                    continue

                img_url = extract_img_url(r.text)
                if not img_url:
                    print("no-img", end=' ')
                    time.sleep(2)
                    continue
                ...
                if img_r.status_code != 200:
                    print(f"dl={img_r.status_code}", end=' ')
                    time.sleep(2)
                    continue
                ...
                if upload_r.status_code == 200:
                    ...
                    print(f"up={upload_r.status_code}", end=' ')
                    time.sleep(2)
            ...
            print("FAILED")
            results[fname] = None
            retry_fails.append(fname)

        time.sleep(0.5)

    # Retry failed ones with longer delay
    if retry_fails:
        print(f"\n=== Retrying {len(retry_fails)} failed ===")
        for fname in retry_fails:
            # ...same logic but with longer delay...
            pass
    ...
    print(f"\n=== Result: {len(success)} success, {len(failed)} failed ===")
