...
4. **VN 框架走 `src/vn/` 新引擎**（剧本驱动，`VnScript` = meta + lines）。指令集：`preload`（`{key,url}` + `wait`）、`say`（`bg`/`cg` + `index`/`zIndex`）、`bg`（cover 占满）、`cg`（contain 看全）、`choice`、`jump`（label / `#hash` / `https://` / 场景名）、`label`、`end`（可 `goto`）。详见 `src/vn/README.md`。旧 `src/avd/*` 是遗留代码，**不要扩展它**。
